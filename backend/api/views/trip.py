...
def get_place_data(place_id, api_key):
    ...
    headers = {
        ...
        'X-Goog-FieldMask': 'id,displayName,photos,location,formattedAddress,rating,websiteUri,editorialSummary'
    }
    ...
    try:
        ...
        if 'editorialSummary' in data and data['editorialSummary']:
            place_data['description'] = data['editorialSummary']['text']
...
def create_place_if_nonexistent(self):
...
class UpdateTripDates(generics.UpdateAPIView):
    serializer_class = TripSerializer
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request, *args, **kwargs):
        with transaction.atomic():
            trip = self.get_object()
            trip = Trip.objects.select_for_update().get(id=trip.id)
            trip.start_date = request.data.get('start_date')
